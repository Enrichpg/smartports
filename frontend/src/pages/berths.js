...
export class BerthsPage {
  ...
  _renderPagination() {
    ...
    if (pages <= 1) return `<div style="font-size:0.82rem;color:var(--sp-text-muted);padding-top:12px">${total} ${t('nav.berths')}</div>`;
    const btns = [];
    ...
    btns.push(`<span class="sp-per-page">${t('ui.page_of')} ${this._page}/${pages} · ${total} ${t('nav.berths')}</span>`);
